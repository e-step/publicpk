...
function addItem(e) {
    ...
    // Validate Input
    if (newItem == '') {
        inputInvalid.innerText = 'Please add an Link';
        return;
    } else {
    ...
    if (isEditMode) {
        ...
        itemToEdit.remove();
        formBtn.innerHTML = "<i class='bi bi-plus'></i> Add Link";
        formBtn.classList.replace('btn-primary', 'btn-dark');

        isEditMode = false;
    } else {
        if (checkIfItemExists(newItem)) {
            inputInvalid.innerText = 'That Link already exists!';
            return;
        } else {
            inputInvalid.innerText = '';
        }
...
function setItemToEdit(item) {
    ...
    formBtn.innerHTML = "<i class='bi bi-pencil-fill'></i> Change Link";
